...
export default function POSView() {
  ...
  const handleGenerateCode = async () => {
    if (!codeFormData.storeId || !codeFormData.posName) {
      setError('Store and POS name are required');
      return;
    }
    ...
    try {
      ...
      setSuccess('Setup code generated successfully!');
    } catch (err: unknown) {
      const error = err as { response?: { data?: { message?: string } } };
  ...
  const handleUpdateStatus = async (id: string, action: 'block' | 'unblock' | 'reset') => {
    const confirmMsg = action === 'reset' 
      ? 'Are you sure you want to reset this device? This will clear its configuration.'
    ...
    try {
      ...
      await loadData();
      setSuccess(`Device ${action}ed successfully!`);
      setTimeout(() => setSuccess(''), 3000);
    } catch (err: unknown) {
      const error = err as { response?: { data?: { message?: string } } };
      setError(error?.response?.data?.message || `Failed to ${action} device`);
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>
            Manage POS devices and generate setup codes
          </p>
        </div>
        <button
        ...
        <div className="rounded-lg border p-4" style={{ backgroundColor: 'var(--surface)', borderColor: 'var(--border)' }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs" style={{ color: 'var(--text-tertiary)' }}>Total Devices</p>
            </div>
          </div>
        </div>
        <div className="rounded-lg border p-4" style={{ backgroundColor: 'var(--surface)', borderColor: 'var(--border)' }}>
        ...
        <div className="rounded-lg border p-4" style={{ backgroundColor: 'var(--surface)', borderColor: 'var(--border)' }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs" style={{ color: 'var(--text-tertiary)' }}>Blocked</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50">
          <div className="w-full max-w-lg rounded-xl shadow-2xl border" style={{ backgroundColor: 'var(--surface)', borderColor: 'var(--border)' }}>
            <div className="flex items-center justify-between p-6 border-b" style={{ borderColor: 'var(--border)' }}>
              ...
              <button onClick={handleCloseCodeModal} className="p-1 rounded-lg transition-colors" style={{ color: 'var(--text-tertiary)' }}>
                <X className="w-5 h-5" />
              </button>
            </div>
                ...
                <div className="space-y-4">
                  <div className="p-4 rounded-lg border" style={{ backgroundColor: 'var(--success-bg)', borderColor: 'var(--success)' }}>
                    <p className="text-sm font-medium mb-3" style={{ color: 'var(--success-text)' }}>
                      Setup code generated successfully!
                    </p>
                  ...
                  <div className="text-sm space-y-2" style={{ color: 'var(--text-secondary)' }}>
                    <p><strong>POS Name:</strong> {generatedCode.posName}</p>
                    <p><strong>Expires At:</strong> {new Date(generatedCode.expiresAt).toLocaleString()}</p>
                  </div>
                </div>
              ) : (
                <>
